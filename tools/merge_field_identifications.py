#!/usr/bin/env python3
"""
Merge field-based identifications into deobfuscated_dump.json.
Only renames classes that are still named ComplexComponent* (doesn't override previous renames).

Usage:
  python tools/merge_field_identifications.py
  python tools/merge_field_identifications.py --dry-run
"""
...
FIELD_RESULTS = BASE_DIR / 'output' / 'field_based_identifications.json'
...
def main():
    ...
    print("=" * 60)
    print("  Merge Field-Based Identifications")
    print("=" * 60)

    with open(FIELD_RESULTS, 'r', encoding='utf-8') as f:
        field_data = json.load(f)

    idents = field_data['identifications']
    # Only take high/medium confidence
    ...
    # Deduplicate: if same VA appears multiple times, take highest confidence
    seen_vas = {}
    for cand in candidates:
        va = cand['va'].upper()
        if va not in seen_vas or (cand['confidence'] == 'high' and seen_vas[va]['confidence'] != 'high'):
            seen_vas[va] = cand
    candidates = list(seen_vas.values())
    print(f"After dedup: {len(candidates)} unique VAs")
    ...
    for cand in candidates:
        ...
        old_name = current
        typ['name'] = cand['suggested_name']
        typ['runtime_source'] = f"FieldAnalysis:{','.join(cand['key_fields'][:3])}"
        renamed += 1
        renames.append((old_name, cand['suggested_name'], cand['confidence']))
